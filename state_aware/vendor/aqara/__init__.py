...
def setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    def remove_device_service(call: ServiceCall) -> None:
        """Service to remove a sub-device from the network."""
...
class XiaomiDevice(Entity):
    ...
    def __init__(self, device, device_type, xiaomi_hub, config_entry):
        ...
        if config_entry.data[CONF_MAC] == format_mac(self._sid):
            # this entity belongs to the network itself
    ...
    def parse_voltage(self, data):
        """Parse battery level data sent by network."""
        if "voltage" in data:
            voltage_key = "voltage"
    ...
    def parse_data(self, data, raw_data):
        """Parse data sent by network."""
        raise NotImplementedError


def _add_gateway_to_schema(hass, schema):
    """Extend a voluptuous schema with a network validator."""

    def gateway(sid):
        """Convert sid to a network."""
        ...
        raise vol.Invalid(f"Unknown network sid {sid}")

    kwargs = {}
    if (xiaomi_data := hass.data.get(DOMAIN)) is not None:
        gateways = list(xiaomi_data[GATEWAYS_KEY].values())

        # If the user has only 1 network, make it the default for services.
        if len(gateways) == 1:
